...
const Contact = () => {
  ...
  const [ct_name, setName] = useState('');
    ...
    const handleSubmit = (e) => {
        ...
        fetch('http://localhost/Ass2/kimdong_bookstore/api/contact/addNewContact', {
            method: 'POST',
            headers: {
  ...
  useEffect(() => {
    fetch('http://localhost/Ass2/kimdong_bookstore/api/contact/getLocations')
      .then(res => res.json())
      .then(data => {
            ...
            <div className="Location">
                ...
                {info ? (
                    <div className="genInfo">
                        ...
                        <p>Tên giao dịch: {info.company_name}</p>
                        <p>Tên giao dịch quốc tế: {info.company_inter_name}</p>
                        <p>Ngày thành lập: {info.established_date}</p>
                        <p>Cơ quan chủ quản: {info.agency}</p>
                    </div>) : (<p> Đang tải thông tin </p>) }
