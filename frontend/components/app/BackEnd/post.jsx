        ...
        if (data !== null) {
            const updatedData = data.map((element) => {
                if (element.id === id) {
                    ...
                    fetch(`https://localhost/BTL_web_1/frontend/components/app/BackEnd/php/uploads/setStatus.php?&url=${encodeURIComponent(element.Page)}&variable=${encodeURIComponent(newStatus)}&id=${encodeURIComponent(id)}`)
        ...
        if (Use === "All") {
            ...
            for (let index = 0; index < checkedItems.length; index++) {
                if (checkedItems[index] === true) {
                    try {
                        const response = fetch(`https://localhost/BTL_web_1/frontend/components/app/BackEnd/php/uploads/delete.php?&id=${encodeURIComponent(data[index].id)}`);
