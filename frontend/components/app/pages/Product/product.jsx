...
export default function Product() {
  ...
  async function loadAndProcessImages() {
    try {
      ...
      if (link === "tat_ca_san_pham") {
        imagePaths = import.meta.glob(
          "./../../BackEnd/php/images/tat_ca_san_pham/**/*.{jpg,jpeg,png,gif,svg,webp}"
        );
      } else if (link === "Kien_thuc_khoa_hoc") {
        imagePaths = import.meta.glob(
          "./../../BackEnd/php/images/kien_thuc_khoa_hoc/**/*.{jpg,jpeg,png,gif,svg,webp}"
        );
      } else if (link === "Lich_su_truyen_thong") {
        imagePaths = import.meta.glob(
          "./../../BackEnd/php/images/lich_su_truyen_thong/**/*.{jpg,jpeg,png,gif,svg,webp}"
        );
      } else if (link === "Truyen_tranh") {
        imagePaths = import.meta.glob(
          "./../../BackEnd/php/images/truyen_tranh/**/*.{jpg,jpeg,png,gif,svg,webp}"
        );
      } else if (link === "Van_hoc_nuoc_ngoai") {
        imagePaths = import.meta.glob(
          "./../../BackEnd/php/images/van_hoc_nuoc_ngoai/**/*.{jpg,jpeg,png,gif,svg,webp}"
        );
      } else if (link === "Van_hoc_Viet_Nam") {
        imagePaths = import.meta.glob(
          "./../../BackEnd/php/images/van_hoc_Viet_Nam/**/*.{jpg,jpeg,png,gif,svg,webp}"
        );
      } else if (link === "Wings_book") {
        imagePaths = import.meta.glob(
          "./../../BackEnd/php/images/wings_book/**/*.{jpg,jpeg,png,gif,svg,webp}"
        );
      }
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `https://localhost/BTL_web_1/frontend/components/app/BackEnd/php/uploads/getFavorite.php?id=${encodeURIComponent(
            element.id
          )}`
        );
  ...
  function HandleMinus() {
    if (soLuong - 1 > 0) setSoLuong(soLuong - 1);
  }
  ...
  useEffect(() => {
    favourite
      ? fetch(
          `https://localhost/BTL_web_1/frontend/components/app/BackEnd/php/uploads/favorite.php?id=${encodeURIComponent(
            element.id
          )}`
        )
      : fetch(
          `https://localhost/BTL_web_1/frontend/components/app/BackEnd/php/uploads/deleteFavorite.php?id=${encodeURIComponent(
            element.id
          )}`
        );
  }, [favourite]);
  ...
  function addToStore() {
    fetch(
      `https://localhost/BTL_web_1/frontend/components/app/BackEnd/php/uploads/store.php?&id=${encodeURIComponent(
        element.id
      )}&sl=${soLuong}`
    );
  }
  ...
  return (
    <div
      className="h-screen bg-cover w-[100%] bg-center"
      ref={parentRef}
      style={{
      ...
      <div className="bg-cover mt-[70px] bg-center h-screen w-[100%] max-lg:pt-[100px] lg:px-[30px] container mx-auto flex justify-center border bg-[white] border-black">
        <div className="w-[100%] flex justify-center mt-[110px] content-center max-lg:block">
          <div className="w-[10%] h-[715px] relative">
                  ...
                  <SwiperSlide>
                    ...
                    >
                      <img
                        ...
                        alt={`Image ${index}`}
                      />
                      <p className="absolute top-0 text-white h-[30px] w-[30px] bg-[#77CBDE] font-bold flex items-center justify-center">
                        {index + 1}
                      </p>
                    </div>
                  </SwiperSlide>
            ...
            <p className="w-full flex justify-center items-center font-bold text-[red]">
              từ {from + 1} đến {from + to}
            </p>
          </div>
          <Swiper
            spaceBetween={30}
          ...
          >
            {image.map((img, ind) => (
              <SwiperSlide
                key={ind}
                className="flex content-center relative items-center justify-center"
              >
            ...
            <div className="items-center break-words">
              <div className="break-words font-sans-serif">
                ...
                <div className="flex items-center">
                  <div>
                    <label className="sm:text-[25px] max-sm:text-[14px]">
                      Rating:{" "}
                    </label>
                    ...
                    <p className="sm:text-[25px] max-sm:text-[14px]">
                      Đã bán: 100
                    </p>
                  </div>
                  <p
                    onClick={handleFavorite}
                    className="cursor-pointer sm:text-[30px] max-sm:text-[20px] pr-[50px] absolute right-0"
                  >
                    <FontAwesomeIcon
                      icon={favourite ? faHeartSolid : faHeartRegular}
                    />
                  </p>
                </div>
              </div>
            ...
            <div className="relative">
              <div>
                ...
                <div className="2xl:flex xl:block">
                  <div className="xl:flex">
                    <div>
                      <label className="text-[red] sm:text-[25px] max-sm:text-[14px] mr-[30px]">
                        <strong>{formatPrice(parseInt(element.gia))}</strong>
                      </label>
                    </div>
                    <div>
                      <label
                        className="text-gray-400 sm:text-[25px] max-sm:text-[14px] line-through"
                        id="original-price"
                      >
                        <strong>
                          {formatPrice(parseInt(element.gia_goc))}
                        </strong>
                      </label>
                    </div>
                  </div>
                  <div>
                    <label
                      className="sm:text-[25px] max-sm:text-[14px] right-0 2xl:absolute xl:block lg:block"
                      id="original-price"
                    >
                      <strong>
                        Bạn đã tiết kiệm được{" "}
                        {formatPrice(
                          parseInt(element.gia_goc) - parseInt(element.gia)
                        )}
                      </strong>
                    </label>
                  </div>
                ...
                <li>
                  <label className="sm:text-[30px] max-sm:text-[20px]">
                    Tác giả:{" "}
                    <strong className="text-[red]">{element.tac_gia}</strong>
                  </label>
                </li>
                <li>
                  <label className="sm:text-[30px] max-sm:text-[20px]">
                ...
                <li>
                  <label className="sm:text-[30px] max-sm:text-[20px]">
                    Khuôn khổ:{" "}
                    <strong className="text-[red]">{element.khuon_kho}</strong>
                  </label>
                </li>
                <li>
                  <label className="sm:text-[30px] max-sm:text-[20px]">
                    Số trang:{" "}
                    <strong className="text-[red]">{element.so_trang}</strong>
                  </label>
                </li>
                <li>
                  <label className="sm:text-[30px] max-sm:text-[20px]">
                    ...
                    <strong className="text-[red]">
                      {formatGram(element.trong_luong)} gram
                    </strong>
                  </label>
                </li>
              </div>
              <div className="left-[72%] top-[130px] xl:absolute lg:block lg:mt-[20px] max-lg:block max-lg:w-[100%] ">
                ...
                <ul className="flex border border-[#8A8C91] xl:w-[200px] lg:w-[100%] h-[50px] mt-[20px] max-lg:w-[100%]">
                  <li
                    className="w-[25%] flex items-center justify-center border border-[#8A8C91] cursor-pointer"
                    onClick={HandleMinus}
                  >
                ...
                <p
                  onClick={addToStore}
                  className="bg-[#FF4086] rounded-lg text-white mt-[20px] py-4 flex text-center justify-center items-center cursor-pointer transition duration-300 ease-in-out hover:-translate-y-1 hover:scale-110 hover:bg-[#FF0000]"
                >
                  THÊM VÀO GIỎ HÀNG
                </p>
                <p className="bg-[#28DD3B] rounded-lg text-white mt-[20px] py-4 flex text-center justify-center items-center cursor-pointer transition duration-300 ease-in-out hover:-translate-y-1 hover:scale-110 hover:bg-[#007F00]">
                  MUA NGAY
                </p>
              </div>
            </div>
