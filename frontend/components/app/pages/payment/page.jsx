...
const SelectMethodPage = () => {
    ...
    const paymentMethods = [
        { id: 'momo', label: 'MoMo', icon: <img src={momoIcon} width={32} height={32} alt="momo" /> },
        { id: 'vnpay', label: 'VNPAY', icon: <img src={vnpayIcon} className="w-8 h-8" alt="vnpay" /> },
        { id: 'zalopay', label: 'ZaloPay', icon: <img src={zalopayIcon} className="w-8 h-8" alt="zalopay" /> },
        { id: 'card', label: 'Credit Card', icon: <CreditCard className="w-8 h-8 text-blue-500" /> },
        { id: 'paypal', label: 'PayPal', icon: <img src={paypalIcon} className="w-8 h-8" alt="paypal" /> },
    ];
        ...
        const fetchData = async () => {
            try {
                ...
                if (!isNaN(parseFloat(id))) {
                    const url = `http://localhost/kimdong_bookstore/frontend/components/app/BackEnd/php/uploads/Data.php?&url=tat_ca_san_pham&variable=${encodeURIComponent(id)}`;
                    response = await fetch(url);
                } else {
                    const url = `https://localhost/kimdong_bookstore/frontend/components/app/BackEnd/php/uploads/getAllStore.php?phone=${encodeURIComponent(ID)}`;
                    response = await fetch(url);
                }
    ...
    let price = store.length > 0 && store.reduce((accumulator, item) => {
        if (!isNaN(parseFloat(id))) {
    ...
    function HandleClick() {
        ...
        const district = districts.find(district => district.Id === selectedDistrict).Name;
        const ward = wards.find(ward => ward.Id === selectedWard).Name;

        if (!name || !email || !address || !phone || !selectedCity || !selectedDistrict || !selectedWard) {
            alert('Vui lòng điền đầy đủ thông tin!');
            return;
        }

        let book = "{";
        const bookEntries = store.map(item => `id: ${item.id}(số lượng: ${item.so_luong ? item.so_luong : so_luong})`);
        book += bookEntries.join(",") + "}";

        let price = store.length > 0 ? store.reduce((accumulator, item) => {
            return accumulator + Number(item.gia) * (item.so_luong ? item.so_luong : so_luong);
        }, 0) : 0;

        const url = `https://localhost/kimdong_bookstore/frontend/components/app/BackEnd/php/uploads/order.php?ho_va_ten=${encodeURIComponent(name)}&email=${encodeURIComponent(email)}&phone=${encodeURIComponent(phone)}&dia_chi=${encodeURIComponent(address)}&thanh_pho=${encodeURIComponent(city)}&huyen=${encodeURIComponent(district)}&xa=${encodeURIComponent(ward)}&total=${price}&book=${encodeURIComponent(book)}`;
        fetch(url)
            .then(response => {
                if (!response.ok) {
                        ...
                        <div className="w-full xl:w-3/4 p-4 border rounded-lg">
                            ...
                            <form>
                                ...
                                <div className='flex w-full justify-end'>
                                    <div onClick={() => { setOpen(true) }} className="bg-blue-500 text-white rounded-lg py-2 px-4 cursor-pointer">
                    ...
                    <div className="lg:pl-[50px] md:pl-[20px] max-md:pl-[20px] md:h-full flex items-center ">
                        <div className='max-md:flex max-sm:hidden'>
                            <Swiper
                                ...
                                direction="vertical"
                                className={`${store.length >= 3 ? "h-[450]" : store.length === 2 ? "h-[300px]" : "h-[150px]"} m-0 md:w-[350px] lg:w-[550px] xl:600px max-md:w-[470px] max-sm:hidden`}
                            >
                                {store.length > 0 && store.map((element, index) => (
                                    <SwiperSlide className="m-0" key={index}>
                                        <div className="w-[300px] flex items-center" href={`/Product/tat_ca_san_pham/${element.id}`}>
                                            <img className="h-[130px]" src={imageStore[index]}></img>
                                            <div className="ml-2 w-[160px]">
                                                <p className="text-[20px] w-[300px]">{element.name}</p>
                                                <p className="text-[14px] w-[300px]">Tập: {element.tap}</p>
                                                <p className="text-[14px] w-[300px]">Giá sản phẩm: {formatPrice(element.gia)}</p>
                                                <p className="text-[14px] w-[300px]">Số lượng: {element.so_luong}</p>
                                            </div>
                                            <div className="text-[red] text-4xl cursor-pointer absolute right-5" onClick={() => { handleClickStore(element.id) }}>x</div>
                                        </div>
                                    </SwiperSlide>
                                ))}
                            </Swiper>
                            <div className='max-md:absolute right-10'>
                                <p>Phí ship: 30.000</p>
                                <p className="text-[20px]">Tạm tính: <label className="font-bold text-[red]"> {formatPrice(price)}</label>
                                    <p className='w-full border-t-2 border-black'>Tổng tiền:<label className="font-bold text-[red]">{formatPrice(price)}</label>
                                    </p>
                                </p>
                            </div>
                    </div>
                ...
                <div className="p-6 flex items-center justify-center bg-gray-100">
                    <div className="max-w-4xl rounded-xl shadow-md p-4 bg-white flex flex-col md:flex-row items-start gap-4">
                        <div className="w-full md:w-1/2 rounded-md space-y-3 max-h-[50vh] overflow-y-auto relative">
                            <h2 className="text-center text-xl font-semibold mb-3 bg-white text-gray-900 sticky top-0 z-10">Choose your payment method</h2>
                            {paymentMethods.map((method) => (
                                <div key={method.id} className="space-y-2">
                                    <label
                                        key={method.id}
                                        htmlFor={method.id}
                                        className={`w-full flex items-center gap-3 p-3 border rounded-md cursor-pointer transition-all ${selectedMethod === method.id ? "border-red-500 bg-red-50" : "border-gray-300 hover:border-gray-400"}`}
                                    >
                                        <input type="radio"
                                            id={method.id}
                                            name="payment"
                                        ...
                                        <div className="w-full rounded-md p-3 border border-gray-200">
                                            <form>
                                                <div className="flex flex-col w-full">
                                                    <label htmlFor="cardNumber" className="text-sm font-bold mb-3">Card Number</label>
                                                    <input id="cardNumber" type="text" placeholder="0000 0000 0000 0000" className='w-full border border-gray-400 rounded-sm p-3' />

                                                </div>
                                                <div className="gap-2 mt-4 flex flex-row">
                                                    <div className="flex flex-col w-full">
                                                        <label htmlFor="date" className="text-sm font-bold mb-3">Expiry date</label>
                                                        <input id="date" type="text" placeholder="MM/YY" className="w-full border border-gray-400 p-2 rounded-sm" />
                                                    </div>
                                                    <div className="flex flex-col w-full">
                                                        <label htmlFor="cvv" className="text-sm font-bold mb-3">Security code</label>
                                                        <input id="cvv" type="text" placeholder="CVV" className="w-full border border-gray-400 p-2 rounded-sm" />
                        ...
                        <div className="max-w-full md:w-1/2 sticky space-y-3 md:pl-6">
                            <h2 className="text-xl text-center font-semibold mb-3 text-gray-900">Order Summary</h2>
                            ...
                            <CustomButton variant='default' rounded='2xl' fullWidth onClick={HandleClick}>
                                Pay now
                            </CustomButton>
