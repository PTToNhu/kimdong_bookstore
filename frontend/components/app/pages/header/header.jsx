...
const listCategory = category.map((element, index) => {
  if (index !== category.length - 1) {
    return (
      <li
        key={index}
        className="border-b  border-black pt-[5px] p-[10px] bg-white hover:text-[red]"
      >
    ...
    return (
      <li
        key={index}
        className="rounded-b-lg border-b border-black pt-[5px] p-[10px] bg-white hover:text-[red]"
      >
        <FontAwesomeIcon icon={faBook} />
        <a className="pl-[5px] ml-[10px]" href={linkCategory[index]}>
          {element}
...
export default function Header(item) {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `http://localhost/BTL_WEB_1/frontend/components/app/BackEnd/php/uploads/getAllFavorite.php`
        );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(
          `http://localhost/BTL_web_1/frontend/components/app/BackEnd/php/uploads/getAllStore.php`
        );
        const data = await response.json();
        setStore(data);
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (indexStore !== -1) {
      fetch(
        `https://localhost/BTL_web_1/frontend/components/app/BackEnd/php/uploads/deleteStore.php?id=${encodeURIComponent(
          indexStore
        )}`
      )
        .then((response) => response.json())
  ...
  const ListFavourite = () => {
    return (
      <div
        ref={resultsRef1}
        className="w-[280px] absolute shadow-2xl p-[4px] mt-[20px] border-2 font-mono border-[#C2C2D9] rounded-sm bg-white text-black right-0 z-50"
      >
    ...
    return (
      ...
      >
        ...
        <div className="flex w-full">
          <a
            className="text-[17px] w-[50%] m-[5px] py-[5px] mb-[5px] flex justify-center items-center bg-[white] text-[red] border border-[red] rounded-sm"
            href="/main/Favorite"
          >
            {" "}
            Xem giỏ hàng
          ...
          <a
            className="text-[17px] w-[50%] m-[5px] py-[5px] mb-[5px] flex justify-center items-center font-bold bg-[red] text-white rounded-sm"
            href="/main/Favorite"
          >
            {" "}
            Thanh toán
  ...
  return (
    <div className="z-100">
      ...
      >
        <div className="flex z-50 pl-[100px] bg-[white] items-center">
          ...
          <ul className="absolute flex p-[30px] pt-[10px] pb-[10px] text-[#009981] text-[20px] right-[3%]">
            <li className="text-[30px] px-[20px] hover:text-[red] cursor-pointer mx-[5px] relative rounded-lg transition delay-150 duration-300 ease-in-out hover:bg-[#EEFFF7]">
              ...
              <p className="absolute top-[-10px] h-[25px] w-[25px] flex items-center justify-center rounded-[50%] bg-[red] right-0 text-white text-[15px]">
                {favourite.length || 0}
              </p>
        ...
        <div
          className={`h-[72px] w-full bg-[black] flex items-center justify-center transition-transform duration-700 ${
        ...
        >
          <div className="flex items-center text-[20px] relative w-[1400px] justify-center">
            ...
            <ul className="text-white flex text-[20px] align-items-center">
              <li className="px-[20px] hover:text-[#15A78A] cursor-pointer flex items-center">
              ...
              <li className="relative px-[20px] cursor-pointer group">
                <a className="group-hover:text-[#15A78A] h-[72px] flex items-center" href="/news">
                  TIN TỨC <FontAwesomeIcon icon={faChevronDown} />
                </a>
                <ul className="absolute left-0 w-[200px] bg-white shadow-md rounded-b-md hidden group-hover:flex flex-col z-50">
                  {news.map((element, index) => (
                    <li
                      key={index}
                      className="border-b border-gray-300 last:rounded-b-md last:border-none"
                    >
                      <a
                        href={linkNews[index]}
                        className="block px-4 py-3 text-black hover:text-[red]"
                      >
                        {element}
                      </a>
                    </li>
