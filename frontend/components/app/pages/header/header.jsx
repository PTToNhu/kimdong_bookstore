...
const listCategory = category.map((element, index) => {
  if (index !== category.length - 1) {
    return (
      <li
        key={index}
        className="border-b  border-black pt-[5px] bg-white hover:text-[red]"
      >
        <FontAwesomeIcon icon={faBook} />
        <a
          className="pl-[5px] ml-[10px]"
          href={linkCategory[index] + "?page=1"}
        >
          {element}
        </a>
      </li>
    ...
    return (
      <li
        key={index}
        className="rounded-b-lg border-b border-black pt-[5px] bg-white hover:text-[red]"
      >
        <FontAwesomeIcon icon={faBook} />
        <a
          className="pl-[5px] ml-[10px]"
...
export default function Header(item) {
  ...
  const fetchDataID = async () => {
    try {
      const response = await fetch(
        `https://localhost/kimdong_bookstore/frontend/components/app/BackEnd/php/uploads/getAllFavorite.php?phone=${encodeURIComponent(
          item.ID
        )}`
      );
  ...
  const fetchData = async () => {
    ...
    if (favoritesArray.length > 0) {
      const promises = favoritesArray.map(async (element_id) => {
        const response = await fetch(
          `https://localhost/kimdong_bookstore/frontend/components/app/BackEnd/php/uploads/getdataFromID.php?variable=${encodeURIComponent(
            element_id
          )}`
        );
        if (!response.ok) {
  ...
  }, [getCookie("Favorite"), item.ID, item.clickFavorite, clickFavorite]);

  const fetchDataStore = async () => {
    try {
      const response = await fetch(
        `https://localhost/kimdong_bookstore/frontend/components/app/BackEnd/php/uploads/getAllStore.php?phone=${encodeURIComponent(
          item.ID
        )}`
      );
      const data = await response.json();
  ...
  const fetchDataStoreCookie = async () => {
    ...
    if (storeArray.length > 0) {
      const promises = storeArray.map(async (element) => {
        ...
        if (match) {
          ...
          const data = await response.json();
          data[0].so_luong = match[2];
          store.push(data[0]);
        } else {
          console.warn(`Không tìm thấy dữ liệu cho phần tử: ${element}`);
        }
      });
  ...
  useEffect(() => {
    if (!item.ID) {
      fetchDataStoreCookie();
    } else {
      fetchDataStore();
    }
  }, [getCookie("Store"), item.ID, item.clickStore, clickStore]);
  ...
  useEffect(() => {
    if (item.ID) {
      ...
      if (indexFavorite !== -1) {
        fetch(
          `https://localhost/kimdong_bookstore/frontend/components/app/BackEnd/php/uploads/deleteFavorite.php?id=${encodeURIComponent(
            indexFavorite
      ...
      if (indexStore !== -1) {
        fetch(
          `https://localhost/kimdong_bookstore/frontend/components/app/BackEnd/php/uploads/deleteStore.php?id=${encodeURIComponent(
            indexStore
          )}&phone=${encodeURIComponent(item.ID)}`
        )
          .then((response) => response.json())
  ...
  function handleClickStore(id) {
    if (item.ID) {
      ...
      const updatedStore = storeArray.filter((storeId) => {
        const regex = new RegExp(`^(${idString})sl\\(\\d+\\)`);
        return !regex.test(storeId);
      });
      document.cookie = "Store=" + updatedStore.join(",") + "; path=/;";
    }
  }
  ...
  const ListFavourite = () => {
    return (
      <div
        ref={resultsRef1}
        className="w-[350px] absolute shadow-2xl p-[4px] mt-[20px] border-2 font-mono border-[#C2C2D9] rounded-sm bg-white text-black right-0 z-150"
      >
        <p className="text-[17px] py-[5px] mb-[5px] flex justify-center items-center bg-[#D51C24] text-white rounded-md">
          {" "}
          ...
          {favourite.length > 0 &&
            favourite.map((element, index) => (
              <SwiperSlide className="m-0" key={index}>
                <div className="flex">
                  <a
                    className="fit object-cover flex items-center"
                  ...
                  <div
                    className="text-[red] cursor-pointer absolute right-1 transition-transform duration-200 hover:scale-200"
                    onClick={() => {
        ...
        <a
          className="text-[17px] py-[5px] mb-[5px] flex justify-center items-center bg-[#17AF91] text-white font-bold rounded-sm"
          href="/main/Favorite?page=1"
        >
          {" "}
          Xem thêm
        </a>
  ...
  const ListStore = () => {
    return (
      <div
        ...
        className="w-[350px] absolute shadow-2xl p-[4px] mt-[20px] border-2 font-mono border-[#C2C2D9] rounded-sm bg-white text-black right-0 z-150"
      >
        <p className="text-[14px] py-[5px] mb-[5px] flex bg-[#D51C24] text-white rounded-md p-3">
          {" "}
          Giỏ hàng của tôi ({store.length || 0} Sản phẩm)
        </p>
        <Swiper
          modules={[Navigation, Pagination, Scrollbar, A11y]}
          spaceBetween={0}
        ...
        <div className="flex w-full">
          <a className="text-[17px] w-[50%] m-[5px] py-[5px] mb-[5px] flex justify-center items-center bg-[white] text-[red] border border-[red] rounded-sm">
            {" "}
            Xem giỏ hàng
          </a>
          <a
  ...
  return (
    <div className="z-150">
      ...
      <header
        id="yourElementId"
        className={`fixed z-50 w-full top-0 left-0 transition-transform duration-700 ${
      ...
      >
        <div className="flex z-50 sm:pl-[100px] max-sm:pl-[30px] bg-[white] items-center">
          <div className="logo flex items-center">
            <img
              src={logoDark}
              className="h-[50px] my-[10px] mr-[50px]"
              alt="Logo"
            />
          </div>
          <ul
            className={`absolute z-100 max-md:hidden block w-[45%] right-[500px]`}
          >
          ...
          <ul className="absolute flex p-[30px] pt-[10px] pb-[10px] text-[#009981] text-[20px] right-[3%]">
            <li className="text-[30px] sm:px-[20px] max-sm:mr-[10px] hover:text-[red] cursor-pointer mx-[5px] sm:relative rounded-lg transition delay-150 duration-300 ease-in-out hover:bg-[#EEFFF7]">
              ...
              <div className="max-sm:relative">
                <p className="absolute top-[0px] max-sm:top-[-50px] h-[25px] w-[25px] flex items-center justify-center rounded-[50%] bg-[red] max-sm:-right-3  right-0 text-white text-[15px]">
                  {favourite.length || 0}
                </p>
              </div>
            ...
            <li
              ...
              className="text-[30px] max-sm:hidden px-[20px] hover:text-[red] cursor-pointer mx-[5px] rounded-lg hover:bg-[#EEFFF7]"
            >
              <FontAwesomeIcon icon={faUser} />
            </li>
          </ul>
          <ul
            className="absolute sm:hidden right-[20px] pl-2"
            onClick={item.handleClickMenu}
          >
        ...
        <div
          className={`h-[51px] max-sm:hidden w-full bg-[black] flex items-center justify-center transition-transform duration-700 ${
        ...
        >
          <div className="flex items-center text-[20px] h-full relative w-[1400px] justify-center">
            <div
              ref={resultsRef3}
              className="flex  items-center w-[300px] absolute left-0 h-full group bg-[#15A78A] "
            >
              <i className="px-[15px] rounded-lg " onClick={clickCategory}>
                ...
                />
                <label
                  className="text-white font-normal px-[10px]"
