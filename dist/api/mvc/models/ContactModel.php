...
class ContactModel extends Database {
    ...
    public function changeStatus($id){
        try {
            $query = $this->conn->prepare("SELECT ct_is_read FROM kdbookstore.contact WHERE ct_id = ?");
            $query->execute([$id]);
            $current = $query->fetch(PDO::FETCH_ASSOC);
    
            if ($current) {
                $newStatus = $current['ct_is_read'] ? 0 : 1;
                $update = $this->conn->prepare("UPDATE kdbookstore.contact SET ct_is_read = ? WHERE ct_id = ?");
    ...
    public function isResponse($id){
        try {
            $query = $this->conn->prepare("SELECT * FROM kdbookstore.contact WHERE ct_id = ?");
            $query->execute([$id]);
            $current = $query->fetch(PDO::FETCH_ASSOC);
    
            if ($current) {
                $newStatus = 1;
                $update = $this->conn->prepare("UPDATE kdbookstore.contact SET ct_is_responsed = ? WHERE ct_id = ?");
                $update->execute([$newStatus, $id]);
                return [
                    'success' => true,
    ...
    public function deleteById($id){
        try {
            ...
            if ($current) {
                $delete = $this->conn->prepare("DELETE FROM kdbookstore.contact WHERE ct_id = ?");
                $delete->execute([$id]);
                return [
                    'success' => true,
    ...
    public function getContacts($num, $page) {
        try {
            ...
            $countQuery = $this->conn->prepare("SELECT COUNT(*) AS total FROM kdbookstore.contact");
            $countQuery->execute();
            ...
            $query = $this->conn->prepare("SELECT * FROM kdbookstore.contact ORDER BY ct_id DESC LIMIT :limit OFFSET :offset");
    ...
    public function getAll() {
        try {
            $query = $this->conn->prepare("SELECT * FROM kdbookstore.contact");
